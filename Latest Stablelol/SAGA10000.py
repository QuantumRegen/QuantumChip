...
print("Kicking off—10,000-Qubit Test for Enhanced Quantum System with Kagome Lattice and Fibonacci Spiral...")

# Parameters
n_qubits = 10000  # Scaling to 10,000 qubits
shots = 64  # Reduced shots for faster sim
time_steps = np.array([1e-6, 7.9e-6, 1e-3])  # Fewer time steps: 1 µs, 7.9 µs, 1 ms
...
creg = ClassicalRegister(2, 'c')  # Still measuring 2 qubits
...
# Simulator setup with MPS method
simulator = AerSimulator(method='matrix_product_state', max_parallel_threads=72, max_parallel_experiments=1)
...
for t in time_steps:
    try:
        ...
        # Measure 2 qubits (same as before, but could scale to more if needed)
        ...
        cx_gates = int((n_qubits - 2) / 3) * 2  # Adjusted for 10,000 qubits
        ...
        plt.xlabel('Count')
        plt.title(f'10,000-Qubit Test at t={t:.1e} s (Fidelity {fidelity_proxy:.3f})')
        plt.xlim(0, shots * 1.1)
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'kagome_fibonacci_test_10000q_t{t:.1e}.png')
        plt.close()
