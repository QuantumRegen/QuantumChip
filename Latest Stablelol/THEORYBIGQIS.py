...
for step in range(1, max(steps_to_save) + 1):
    ...
    # Gravitational Collapse (Slightly tighter focus)
    for i in range(n_qubits):
        qc.rz(-1.0 * np.exp(-((i - n_qubits / 2) / 3.5) ** 2), i)

    # Pulse (Steps 50-52)
    if 50 <= step <= 52:
        qc.rx(-2.0 * np.cos(100 * t), range(n_qubits))

    # Quantum Foam (Add controlled entanglement)
    if 50 <= step <= 52:
        for i in range(0, n_qubits - 1):
            qc.cz(i, i + 1)
        for i in range(0, n_qubits - 2, 2):
            qc.cz(i, i + 2)
        # Controlled third layer
        for i in range(0, n_qubits - 3, 4):
            qc.cz(i, i + 3) if i % 2 == 0 else None

    # Dark Matter Geometry
    rho_sim = step / 500
    for i in range(n_qubits):
        qc.rz(1.5 * rho_sim * (i / n_qubits), i)

    # Vortex (Wayne Pulse with stronger feedback)
    if step > 200:
        qc.rx(1.5, range(n_qubits))
        if step == 201:
            qc.reset(range(n_qubits // 2, n_qubits))
            qc.h(range(n_qubits // 2, n_qubits))
        for i in range(n_qubits):
            qc.rz(0.7 * (i - n_qubits / 2) / n_qubits, i)
