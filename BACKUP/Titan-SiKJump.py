...
def hamiltonian(t):
    ...
    for i in range(qubits):
        if i == 0:
            H_local = Z + b_field_strength * X - damping_factor * I + 0.1 * Z  # ZZ with qubit 1
            H.append(add_noise(H_local, noise_scale))
        elif i == 1:
            H_local = Z + 0.1 * Z - damping_factor * I  # Matching ZZ
            H.append(add_noise(H_local, noise_scale))
        elif i % 3 == 0:
...
def get_rho_two_qubits(nodes):
    ...
    rho = rho.reshape(4, 4)  # Reshape to 4x4 matrix
...
for t in range(steps):
    ...
    rho_two_reduced = np.trace(rho_two.reshape(2, 2, 2, 2), axis1=2, axis2=3)  # Partial trace over second qubit
...
plt.savefig('titan_sik_simulation_with_entanglement_fixed_bell.png')
plt.show()
