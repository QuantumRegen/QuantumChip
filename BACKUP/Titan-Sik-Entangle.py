...
def hamiltonian(t):
    ...
    for i in range(qubits):
        if i == 0:
            H_local = Z + b_field_strength * X - damping_factor * I + 0.1 * Z
            H.append(add_noise(H_local, noise_scale))
        elif i == 1:
            H_local = Z + 0.1 * Z - damping_factor * I
            H.append(add_noise(H_local, noise_scale))
        elif i % 3 == 0:
...
for t in range(steps):
    ...
    # Now define uptime based on entanglement
    uptime_flag = 1 if entanglement > 0.5 else 0  # Entanglement-based uptime
...
plt.savefig('titan_sik_simulation_fixed_name_error.png')
plt.show()
